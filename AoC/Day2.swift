...
//  AoC
//
//  Created by Laksh Gandikota on 12/2/20.
//
...
"""
    ...
    init() {
        let passwords = day2.split(separator: "\n")

        for password in passwords {
            //part1.answer += verifyPasswordPolicyPart1(String(password))
            part2.answer += verifyPasswordPolicyPart2(String(password))
        }
    }
    
    func verifyPasswordPolicyPart1(_ text: String) -> Int64 {
        let problem = text.split(separator: " ")
        let range = problem[0].split(separator: "-")
        let letter = problem[1].split(separator: ":")
        
        var matches: Int = 0
        
        for eachLetter in problem[2] {
            
            if (letter[0] == String(eachLetter)) {
                matches += 1
            }
        }
        
        if matches >= Int(range[0]) ?? 0 && matches <= Int(range[1]) ?? 0 {
            return 1
        }
        return 0
    }
    
    func verifyPasswordPolicyPart2(_ text: String) -> Int64 {
        let problem = text.split(separator: " ")
        let range = problem[0].split(separator: "-")
        let letter = problem[1].split(separator: ":")
        let password = Array(problem[2])

        let startIndex: Int = Int(range[0])! - 1
        let endIndex: Int = Int(range[1])! - 1
        
        let character: String = String(letter[0])

        if ((character == String(password[startIndex]) || character == String(password[endIndex])) && ( String(password[startIndex]) !=  String(password[endIndex]))   ) {
            return 1
        }
